...
export const PAYPAL_CONFIG = new InjectionToken<PayPalConfig>('');

/** PayPal initializer token */
export const PAYPAL_INSTANCE = new InjectionToken<Promise<PayPal>>('');
...
export function loadPayPalSdk(config: PayPalConfig): Promise<PayPal> {
  ...
  return payPal ? Promise.resolve( payPal ) : new Promise( (resolve, reject) => {
    ...
    try {
      ...
      console.log(payPalSdkUrl(config));
...
export function payPalSdkUrl(config: PayPalConfig): string {
  ...
  const url = 'https://www.paypal.com/sdk/js?client-id=' + id
  return url;
  // return Object.keys(config).reduce( (url, token, idx) => {
  //   return url + (idx && '&' || '') + hyphenize(token) + '=' + config['clientId'];
  // }, PAYPAL_JS_SDK);
}